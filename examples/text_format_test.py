...
def check_reportlab_installation():
    ...
    try:
        import reportlab
        print(f"✅ ReportLab found: version {reportlab.Version if hasattr(reportlab, 'Version') else 'Unknown'}")
        return True
    except ImportError:
...
def test_text_format_parsing(file_path: str):
    ...
    try:
        ...
        print("⚠️  Text encoding: Non-UTF-8 (will try multiple encodings during processing)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test text format parsing with MinerU"
    )
    parser.add_argument(
        "--file", 
        required=True, 
        help="Path to the text file to test"
    )
    parser.add_argument(
        "--check-reportlab", 
        action="store_true", 
